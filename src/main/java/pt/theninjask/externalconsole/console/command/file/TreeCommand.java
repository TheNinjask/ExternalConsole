...
public class TreeCommand implements ExternalConsoleCommand {
    ...
    private final Map<Option, Runnable> optionsMap = Map.ofEntries(
    ...
    private void printDirectoryTree(Path dir, boolean includeFiles, String... fileExtensions) {
        ...
        FileFilter filter = dir1 -> dir1.isDirectory() || (includeFiles
                ...
                || Arrays.stream(fileExtensions).anyMatch(ext -> dir1.getName().endsWith(ext))));
        ...
        for (int i = 0; i < ((File[]) Objects.requireNonNull(linkedList[ARRAY])).length; i++) {
            ...
            if (sub.isDirectory() && Objects.requireNonNull(files).length != 0) {
    ...
    public int executeCommand(String... args) {
        try {
            ...
            optionsMap.keySet().forEach(tree::addOption);
            options.addOptionGroup(tree);

            CommandLineParser parser = new DefaultParser();
            CommandLine cmd = parser.parse(options, args);
            switch (cmd.getArgs().length) {
                case 0 -> printDirectoryTree(getCurrentDir(), cmd.hasOption('f'), cmd.getOptionValues('f'));
                default -> {
